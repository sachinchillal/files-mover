...
import { browseDestinationTree } from '../../src/fs/browse';
...
describe(`POST ${API_BASE_URL}copy`, () => {
  beforeEach(() => {
    createSourceFixture();
    writeTestSettings(SOURCE_TEST_DIR, DESTINATION_TEST_DIR);
  });

  it('copies source folder as a subfolder of destination', async () => {
    const res = await request(app)
      .post(`${API_BASE_URL}copy`)
    ...
    expect(res.body.copiedFiles).toBeGreaterThan(0);
    expect(res.body.copiedDirectories).toBeGreaterThan(0);
  ...
  it('skips ignored directories during copy', async () => {
    const res = await request(app)
      .post(`${API_BASE_URL}copy`)
      .send(
        withPairBody({
  ...
  it('returns 404 when destination is missing', async () => {
    const res = await request(app)
      .post(`${API_BASE_URL}copy`)
      .send(withPairBody({ source: 'projects/app', destination: 'missing/path' }));

    expect(res.status).toBe(404);
    expect(res.body.message).toBe('Destination not found');
  });

  it('returns 400 when source equals destination within the same root', async () => {
    ...
    const res = await request(app)
      .post(`${API_BASE_URL}copy`)
      .send(withPairBody({ source: 'projects/app', destination: 'projects/app' }));

    expect(res.status).toBe(400);
    expect(res.body.message).toBe('Source and destination cannot be the same');
  });
  ...
  it('returns 400 when destination is under source within the same root', async () => {
    ...
    expect(res.status).toBe(400);
    expect(res.body.message).toBe('Cannot copy into source');
  });

  it('returns 400 for invalid path with ..', async () => {
    const res = await request(app)
      .post(`${API_BASE_URL}copy`)
      .send(withPairBody({ source: 'projects/../', destination: 'backups' }));

    expect(res.status).toBe(400);
    expect(res.body.message).toBe('Incorrect path');
  });

  it('merges into existing subfolder on re-copy', async () => {
    await request(app)
      .post(`${API_BASE_URL}copy`)
      .send(withPairBody({ source: 'projects/app', destination: 'backups' }));
    ...
    const srcDir = path.join(DESTINATION_TEST_DIR, 'backups/app/src');
    expect(fs.existsSync(srcDir)).toBe(true);
    expect(fs.readdirSync(path.join(DESTINATION_TEST_DIR, 'backups/app')).filter((n) => n === 'src')).toHaveLength(1);
  });

  it('copies multiple source folders into destination', async () => {
    const res = await request(app)
      .post(`${API_BASE_URL}copy`)
  ...
  it('copies a single file into destination folder', async () => {
    const res = await request(app)
      .post(`${API_BASE_URL}copy`)
      .send(withPairBody({ sources: ['projects/app/readme.md'], destination: 'backups' }));

    expect(res.status).toBe(200);
    expect(res.body.copiedFiles).toBe(1);
    expect(res.body.copiedDirectories).toBe(0);
  ...
  it('returns 400 when sources array is empty', async () => {
    const res = await request(app)
      .post(`${API_BASE_URL}copy`)
      .send(withPairBody({ sources: [], destination: 'backups' }));

    expect(res.status).toBe(400);
    expect(res.body.message).toBe('No sources selected');
  });

  it('returns 400 when source path is empty string', async () => {
    const res = await request(app)
      .post(`${API_BASE_URL}copy`)
      .send(withPairBody({ sources: [''], destination: 'backups' }));
  ...
  it('copies only selected files without creating a root folder', async () => {
    const res = await request(app)
      .post(`${API_BASE_URL}copy`)
      .send(withPairBody({ sources: ['readme.md', 'license.txt'], destination: 'backups' }));

    expect(res.status).toBe(200);
    expect(res.body.copiedFiles).toBe(2);
    expect(res.body.copiedDirectories).toBe(0);

    expect(fs.existsSync(path.join(DESTINATION_TEST_DIR, 'backups/readme.md'))).toBe(true);
    expect(fs.existsSync(path.join(DESTINATION_TEST_DIR, 'backups/license.txt'))).toBe(true);
    expect(fs.existsSync(path.join(DESTINATION_TEST_DIR, 'backups/root'))).toBe(false);
  });

  it('returns 404 for denied file types', async () => {
    const res = await request(app)
      .post(`${API_BASE_URL}copy`)
      .send(withPairBody({ sources: ['invalid-file-type.lock'], destination: 'backups' }));
  ...
  it('returns 400 when pairId is missing', async () => {
    const res = await request(app)
      .post(`${API_BASE_URL}copy`)
      .send({ source: 'projects/app', destination: 'backups' });
